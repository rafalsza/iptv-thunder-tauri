...
interface FavoriteMovieCategoryCardProps {
  category: StalkerGenre;
  categoryIndex: number;
  selectedCategory: StalkerGenre | null;
  onSelect: (category: StalkerGenre) => void;
  onToggleFavorite: (e: React.MouseEvent, categoryId: string) => void;
  onLongPress: (category: StalkerGenre) => void;
}

const FavoriteMovieCategoryCard: React.FC<FavoriteMovieCategoryCardProps> = ({
  category,
  categoryIndex,
  selectedCategory,
  onSelect,
  ...
  return (
    <div
      key={category.id}
      data-tv-focusable
      data-tv-group="favorite-movie-categories"
...
export const FavoriteMovieCategoriesList: React.FC<FavoriteMovieCategoriesListProps> = ({
    ...
    return (
      <div className="flex-1 flex items-center justify-center">
        <div className="text-center dark:text-white text-slate-900">
          ...
          <h3 className="text-xl font-semibold mb-2">Brak ulubionych kategorii filmów</h3>
          <p className="dark:text-slate-400 text-slate-600 mb-4">
            Nie masz jeszcze żadnych ulubionych kategorii filmów. Dodaj je klikając serce ❤️ w widoku kategorii filmów.
          </p>
          <div className="mt-6 p-4 dark:bg-slate-800 dark:bg-opacity-50 bg-white bg-opacity-50 rounded-lg">
            <p className="text-sm dark:text-slate-400 text-slate-600">
              💡 <strong>Wskazówka:</strong> Przejdź do <strong>Movies → Categories</strong> i kliknij serce przy kategoriach, które chcesz dodać do ulubionych.
            </p>
          </div>
        </div>
      </div>
  ...
  return (
    <div className="flex-1 flex flex-col overflow-hidden">
      ...
      <div className="border-b dark:border-slate-700 border-gray-300 p-4">
        <h1 className="text-lg font-bold dark:text-white text-slate-900">{t('favoriteMovieCategories')}</h1>
        <p className="text-sm dark:text-slate-400 text-slate-600">
