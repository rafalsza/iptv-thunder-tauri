...
export const PortalForm: React.FC<PortalFormProps> = ({ portal, onClose }) => {
  ...
  return (
    <div className="fixed inset-0 bg-black bg-opacity-75 flex items-center justify-center z-50 p-4">
      <div className="bg-gradient-to-br from-slate-800 to-slate-900 rounded-2xl shadow-2xl w-full max-w-2xl max-h-[90vh] overflow-y-auto border border-slate-600">
        {/* Header */}
        ...
        <form onSubmit={handleSubmit} className="p-6 space-y-6">
          ...
          <div className="space-y-4">
            ...
            <div>
              ...
              <input
                ...
                className={`w-full px-4 py-3 bg-slate-700 bg-opacity-50 border rounded-lg focus:ring-2 focus:ring-blue-500 focus:border-blue-500 text-white placeholder-slate-400 ${
            ...
            <div>
              ...
              <textarea
                ...
                rows={3}
                className="w-full px-4 py-3 bg-slate-700 bg-opacity-50 border border-slate-600 rounded-lg focus:ring-2 focus:ring-blue-500 focus:border-blue-500 text-white placeholder-slate-400"
            ...
            <div>
              ...
              <div className="flex flex-wrap gap-2 mb-2">
                {formData.tags?.map((tag, index) => (
                  <span
                    key={`${tag}-${index}`}
                    className="px-3 py-1 bg-blue-500 bg-opacity-20 text-blue-400 rounded-full text-sm flex items-center gap-2 border border-blue-400 border-opacity-30"
                  >
                    {tag}
                    <button
                      type="button"
          ...
          <div className="space-y-4">
            ...
            <div>
              ...
              <input
                ...
                className={`w-full px-4 py-3 bg-slate-700 bg-opacity-50 border rounded-lg focus:ring-2 focus:ring-blue-500 focus:border-blue-500 text-white placeholder-slate-400 ${
                  errors.portalUrl ? 'border-red-500' : 'border-slate-600'
                }`}
                placeholder="http://portal.example.com/"
              />
              {errors.portalUrl && (
                <p className="mt-1 text-sm text-red-400">{errors.portalUrl}</p>
              )}
            </div>
            ...
            <div>
              ...
              <input
                ...
                className={`w-full px-4 py-3 bg-slate-700 bg-opacity-50 border rounded-lg focus:ring-2 focus:ring-blue-500 focus:border-blue-500 text-white placeholder-slate-400 font-mono ${
                  errors.mac ? 'border-red-500' : 'border-slate-600'
                }`}
                placeholder="00:1A:79:84:1A:AB"
              />
              {errors.mac && (
                <p className="mt-1 text-sm text-red-400">{errors.mac}</p>
              )}
          ...
          <div className="flex justify-end gap-3 pt-6 border-t border-slate-700">
            ...
            <button
              ...
              className="px-6 py-3 bg-gradient-to-r from-blue-600 to-purple-600 text-white rounded-lg hover:from-blue-700 hover:to-purple-700 disabled:bg-slate-600 disabled:cursor-not-allowed transition-all duration-200 transform hover:scale-105 flex items-center gap-2 shadow-lg"
            >
              {isSubmitting ? (
                <>
