...
export const FavoriteCategoriesList: React.FC<FavoriteCategoriesListProps> = ({
  ...
  // Pobieranie wszystkich kategorii kanałów z SQLite cache
  const {
    data: categories = [],
    isLoading,
    error,
      ...
      <div className="px-4 pt-4 pb-2">
        <div className="flex items-center gap-3">
          ...
          <div>
            ...
            <p className="text-sm dark:text-slate-400 text-slate-600">
              {t('yourFavoriteChannelCategories')} · <span className="dark:text-green-400 text-green-700 font-medium">{favoriteCategories.length}</span>
            </p>
          </div>
        </div>
        {search && (
